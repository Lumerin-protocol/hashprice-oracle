...
Publishing happens from CI (see `.github/workflows/publish-oracle-abi.yml`). A `dev` publish updates `environments.testnet` and is tagged `dev` (minor bump). A `main` publish updates `environments.mainnet` and is tagged `latest`; the first mainnet cut is `3.0.0`, skipping any major npm has already used.
